  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${
        scrolled
          ? "border-b border-[var(--color-border-soft)] bg-[#0d0b09]/80 backdrop-blur-xl"
    ...
    >
      <nav className="container-x flex h-[88px] items-center justify-between gap-6">
        <Link href="/" className="flex items-center gap-2.5" aria-label={`${siteConfig.name} home`}>
          <span className="relative grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-secondary)] text-[#1c0d05]">
        ...
        <div className="hidden items-center gap-8 md:flex">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="group relative text-[0.975rem] font-medium text-[var(--color-muted)] transition-colors hover:text-[var(--color-text)]"
            >
              {l.label}
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            className="fixed inset-x-0 top-[88px] z-50 border-b border-[var(--color-border)] bg-[#0d0b09]/98 backdrop-blur-xl md:hidden"
          >
