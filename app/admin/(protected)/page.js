      ...
      <nav className="mt-8 flex flex-wrap gap-2">
        {["all", ...STATUSES].map((key) => {
          ...
          return (
            ...
            >
              {LABELS[key]}
              <span className="rounded-full bg-white/[0.06] px-2 py-0.5 text-xs tabular-nums">
                {count}
              </span>
            </Link>
          );
...
function PageLink({ status, page, disabled, children }) {
  ...
  if (disabled) {
    return (
      <span className="rounded-full border border-[var(--color-border-soft)] px-4 py-2 text-sm text-[var(--color-dim)] opacity-50">
        {children}
      </span>
    );
  }
  ...
  return (
    <Link
      href={href}
      className="rounded-full border border-[var(--color-border)] px-4 py-2 text-sm text-[var(--color-muted)] transition-colors hover:border-[var(--color-primary)] hover:text-[var(--color-text)]"
    >
      {children}
    </Link>
  );
